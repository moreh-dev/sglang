...
from tqdm import tqdm as tqdm_sync  # 동기 tqdm (write용)
...
def build_result_details(
    outputs: List[RequestFuncOutput], output_lens: List[int]
) -> Dict[str, Any]:
    return {
...
def append_result_file(
    ...
    record = result | result_details if args.output_details else result
    with open(output_file_name, "a") as file:
        file.write(json.dumps(record) + "\n")


def log_line(message: str, *, pbar=None) -> None:
    if pbar is not None:
        tqdm_sync.write(message)
    else:
        print(message)
    sys.stdout.flush()


def _resolve_wandb_key(args) -> Optional[str]:
    ...
    if env_key:
        setattr(args, "wandb_key", env_key)
        return env_key
    ...
    try:
        ...
        if os.path.exists(netrc_path):
            ...
            authenticators = netrc_file.authenticators("api.wandb.ai")
            if authenticators and authenticators[2]:
                setattr(args, "wandb_key", authenticators[2])
...
def _init_wandb_session(args) -> Optional[Any]:
    if getattr(args, "wandb_project", None) is None:
        return None

    if _wandb_module is None:
        raise ImportError(
            "wandb is not installed. Please install wandb to enable wandb logging."
        )

    wandb_key = _resolve_wandb_key(args)
    if wandb_key is None:
        raise ValueError(
            "To enable wandb logging, provide an API key via --wandb-key, WANDB_API_KEY "
    ...
    _wandb_module.login(key=wandb_key)

    created_new_run = False
    if _wandb_module.run is None:
        _wandb_module.init(**init_kwargs)
        created_new_run = True

    setattr(_wandb_module, "_sglang_created_run", created_new_run)
...
def print_metrics(
    ...
    banner = header if header is not None else " Serving Benchmark Result "
    log_line("\n{s:{c}^{n}}".format(s=banner, n=50, c="="), pbar=pbar)
    log_line("{:<40} {:<10}".format("Backend:", backend), pbar=pbar)
    if weight_version is not None:
        log_line("{:<40} {:<10}".format("Weight version:", weight_version), pbar=pbar)
    log_line("{:<40} {:<10}".format("Traffic request rate:", request_rate), pbar=pbar)
    log_line(
        "{:<40} {:<10}".format(
            "Max request concurrency:",
            max_concurrency if max_concurrency is not None else "not set",
        ),
        pbar=pbar,
    )
    log_line(
        "{:<40} {:<10}".format("Successful requests:", metrics.completed), pbar=pbar
    )
    log_line("{:<40} {:<10.2f}".format("Benchmark duration (s):", duration), pbar=pbar)
    log_line(
        "{:<40} {:<10}".format("Total input tokens:", metrics.total_input), pbar=pbar
    )
    log_line(
    ...
    log_line("{:<40} {:<10.2f}".format("Concurrency:", metrics.concurrency), pbar=pbar)
    if accept_length:
        log_line("{:<40} {:<10.2f}".format("Accept length:", accept_length), pbar=pbar)
    log_line("{s:{c}^{n}}".format(s="End-to-End Latency", n=50, c="-"), pbar=pbar)
    log_line(
        "{:<40} {:<10.2f}".format(
    ...
    log_line("{s:{c}^{n}}".format(s="Time to First Token", n=50, c="-"), pbar=pbar)
    log_line(
        "{:<40} {:<10.2f}".format("Mean TTFT (ms):", metrics.mean_ttft_ms), pbar=pbar
    )
    log_line(
    ...
    log_line(
        "{:<40} {:<10.2f}".format("P99 TTFT (ms):", metrics.p99_ttft_ms), pbar=pbar
    )
    log_line("{s:{c}^{n}}".format(s="Inter-Token Latency", n=50, c="-"), pbar=pbar)
    log_line(
        "{:<40} {:<10.2f}".format("Mean ITL (ms):", metrics.mean_itl_ms), pbar=pbar
    )
    log_line(
        "{:<40} {:<10.2f}".format("Median ITL (ms):", metrics.median_itl_ms), pbar=pbar
    )
    log_line("{:<40} {:<10.2f}".format("P95 ITL (ms):", metrics.p95_itl_ms), pbar=pbar)
    log_line("{:<40} {:<10.2f}".format("P99 ITL (ms):", metrics.p99_itl_ms), pbar=pbar)
    log_line("{:<40} {:<10.2f}".format("Max ITL (ms):", metrics.max_itl_ms), pbar=pbar)
    log_line("=" * 50, pbar=pbar)

    if wandb_logger is not None:
        if not hasattr(wandb_logger, "log"):
            log_line(
                "Provided wandb argument has no `log` attribute; skipping wandb logging.",
                pbar=pbar,
            )
            return
        ...
        wandb_metrics = {
            ...
            # "total_token_throughput": metrics.total_throughput,
            "output_token_throughput_retokenized": metrics.output_throughput_retokenized,
            # "total_token_throughput_retokenized": metrics.total_throughput_retokenized,
            "mean_ttft_ms": metrics.mean_ttft_ms,
            # "median_ttft_ms": metrics.median_ttft_ms,
            # "std_ttft_ms": metrics.std_ttft_ms,
            # "p99_ttft_ms": metrics.p99_ttft_ms,
        ...
        if accept_length is not None:
            wandb_metrics["accept_length"] = accept_length
        if weight_version is not None:
            wandb_metrics["weight_version"] = weight_version
        # if header is not None:
        #     wandb_metrics["metrics_header"] = header
        # wandb_metrics["result_type"] = "interval" if header is not None else "final"
        ...
        try:
            ...
            log_line(f"Failed to log metrics to wandb: {exc}", pbar=pbar)


def get_accept_length_weight_version_from_server(
    backend: str,
    base_url: str,
):
    if "sglang" in backend:
        ...
        if server_info.status_code == 200:
            ...
            weight_version = server_info_json.get("weight_version", None)
        else:
            accept_length = None
            weight_version = None
    else:
    ...
    return accept_length, weight_version
...
async def benchmark(
    ...
    if not log_interval:
        ...
        last_logged_time = benchmark_start_time
        while pending_tasks:
            done, pending_tasks = await asyncio.wait(
                pending_tasks, timeout=log_interval
            )
            if done:
                now = time.perf_counter()
                outputs.extend([task.result() for task in done])
                if outputs and len(outputs) > last_logged_completed:
                    elapsed = max(now - benchmark_start_time, 1e-6)
                    duration = now - last_logged_time
                    metrics, output_lens = calculate_metrics(
                        outputs=outputs[last_logged_completed:],
                        dur_s=duration,
                        tokenizer=tokenizer,
                    )
                    input_toks_check += metrics.total_input
                    num_requests_check += metrics.completed
                    interval_header = (
                        f" Serving Benchmark Result (elapsed {elapsed:.2f}s) "
                    )
                    accept_length, weight_version = (
                        get_accept_length_weight_version_from_server(backend, base_url)
                    )
                    print_metrics(
                        metrics=metrics,
                        backend=backend,
                    ...
                    result["result_type"] = "interval"
                    result["observed_requests"] = len(outputs)
                    result_details = build_result_details(outputs, output_lens)
                    append_result_file(output_file_name, result, result_details)
                    last_logged_completed = len(outputs)
    ...
    accept_length, weight_version = get_accept_length_weight_version_from_server(
        backend, base_url
    )
...
def run_benchmark(args_: argparse.Namespace, wandb_logger: Optional[Any] = None):
    global args
    args = args_
    ...
    for attr in ("wandb_project", "wandb_name", "wandb_key"):
    ...
    if wandb_logger is None and args.wandb_project:
        wandb_logger = _init_wandb_session(args)

    wandb_session_initialized = False
    if wandb_logger is not None:
        wandb_session_initialized = getattr(wandb_logger, "_sglang_created_run", False)
    ...
    try:
        ...
        if wandb_session_initialized and wandb_logger is not None:
            try:
                current_run = getattr(wandb_logger, "run", None)
                if current_run:
                    wandb_logger.finish()
            except Exception as exc:
                print(f"Failed to close wandb session: {exc}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Interval (in seconds) to log intermediate metrics during the benchmark run",
    )
    parser.add_argument(
        "--wandb-project",
        type=str,
        default=None,
        help="Weights & Biases project to log benchmark metrics.",
    )
    parser.add_argument(
        "--wandb-name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Weights & Biases API key. If omitted, uses WANDB_API_KEY or ~/.netrc.",
    )
